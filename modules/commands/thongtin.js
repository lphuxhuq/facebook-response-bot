...
module.exports.run = async function ({ api, event, args, Users, permssion, getText ,Threads}) {
  ...
  const listNDH = NDH || config.NDH ||  [];
  {
    ...
    var callback = () => 
      api.sendMessage({ body: `==「 ${namebot} 」==\n\n[🔰] 𝐏𝐫𝐞𝐟𝐢𝐱 𝐡𝐞̣̂ 𝐭𝐡𝐨̂́𝐧𝐠: ${PREFIX}\n[📛] 𝐏𝐫𝐞𝐟𝐢𝐱 𝐛𝐨𝐱: ${prefix}\n[📱] 𝐌𝐨𝐝𝐮𝐥𝐞𝐬: ${commands.size}\n[🌐] 𝐏𝐢𝐧𝐠: ${Date.now() - dateNow}ms\n[📈] 𝐅𝐚𝐬𝐭: ${resault} 𝐌𝐁𝐒\n──────────────\n======「 𝐀𝐃𝐌𝐈𝐍 」 ======\n${msg.join("\n")}\n──────────────\n===「 𝐍𝐆𝐔̛𝐎̛̀𝐈 𝐇𝐎̂̃ 𝐓𝐑𝐎̛̣ 」 ===\n${msg1.join("\n")}\n──────────────\n𝐓𝐡𝐨̛̀𝐢 𝐠𝐢𝐚𝐧 𝐁𝐨𝐭 𝐨𝐧𝐥𝐢𝐧𝐞 ${hours} 𝐡𝐨𝐮𝐫(𝐬) ${minutes} 𝐦𝐢𝐧𝐮𝐭𝐞(𝐬) ${seconds} 𝐬𝐞𝐜𝐨𝐧𝐝(𝐬)\n\n» 𝐓𝐨𝐭𝐚𝐥 𝐮𝐬𝐞𝐫𝐬: ${global.data.allUserID.length} \n» 𝐓𝐨𝐭𝐚𝐥 𝐭𝐡𝐫𝐞𝐚𝐝𝐬: ${global.data.allThreadID.length}\n──────────────\n[𝐁𝐚̣𝐧 𝐜𝐨́ 𝐛𝐢𝐞̂́𝐭 ?]: ${data[Math.floor(Math.random() * data.length)]}`, attachment: fs.createReadStream(__dirname + "/cache/" + ((typeof event !== 'undefined' && event) ? (event._tempId || (event._tempId = Date.now() + '_' + Math.random().toString(36).substring(2, 6))) : Date.now()) + "_nah.jpg"), }, event.threadID, () => fs.unlinkSync(__dirname + "/cache/" + ((typeof event !== 'undefined' && event) ? (event._tempId || (event._tempId = Date.now() + '_' + Math.random().toString(36).substring(2, 6))) : Date.now()) + "_nah.jpg"));
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/cache/" + ((typeof event !== 'undefined' && event) ? (event._tempId || (event._tempId = Date.now() + '_' + Math.random().toString(36).substring(2, 6))) : Date.now()) + "_nah.jpg")).on("close", () => callback()); 
  }
};